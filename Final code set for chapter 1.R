    ...
    beta <- 20 ##The data I sent you, which is called data in R is the 1000 realisations of these parameters
    theta <- 25
    b <- 1
    area.host<-1
    infbegin<-1
    ...
    #########################################################################
    sel1<-sample(data$sim,4)
    datarow<-data%>%filter(sim==sel1)
    ggplot(datarow,aes(x=x,y=y))+geom_point()+facet_wrap(~sim)
