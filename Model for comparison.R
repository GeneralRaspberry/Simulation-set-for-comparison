...
## meta parameters
delta.t <- 400 # time step (ALEX-THIS IS BIGGER THAN THE EXPERIMENT BELOW BECAUSE IT IS TAKING SO MUCH LONGER!)
...
beta <- 0.002 ##The data I sent you, which is called data in R is the 1000 realisations of these parameters
theta <- 5
b <- 1
area.host<-1
infbegin<-1
...
ggplot(data_log) + geom_line(aes(x=time, y=infected/hosts, group=sim), size=.2,colour="gray70") +
  ...
  ggtitle(paste0("Epidemic growth curve for 1000 simulations"))+theme_tufte()+xlim(0,max(times)) +
...
wdspec1<-"C:/Users/owner/Documents/Uni stuff/PhD/R scripts/Chapter 1/Simulation set/Raw Data/"
datanamefile<-paste0(wdspec1,"Theta", theta, " Beta", beta, " Rf", randmod, " Delta t", delta.t,"growth rate", mean_r, ".Rda")
save(data,beta,theta,randmod,delta.t,mean_r, file=datanamefile)
